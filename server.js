...
async function gerar(req,res){
  ...
  }catch(e){ console.log("ERRO GERAR", e); res.status(500).json({erro:e.mensagem || e.message || e}) }
}
  ...
  try{
    ...
    for(const p of pix){
      if(p.txid){
        ...
        if(item){ item.status='PAGO_LIBERAR'; salvaFila(); console.log(`PAGO ${p.txid}`); }
      }
    }
...
app.get('/verifica/:txid', async (req,res)=>{
  try{
    ...
    if(r.status==='CONCLUIDA'){
      ...
      if(item){ item.status='PAGO_LIBERAR'; salvaFila(); }
    }
    res.json(r);
