...
.logo{color:#fff;font-weight:900;font-size:22px}
...
.plano{background:#1a335f;border:2px solid #2a4a85;border-radius:12px;padding:12px 15px;margin:10px 0;display:flex;justify-content:space-between;color:#fff;cursor:pointer}
...
<div class="aviso">NAO FECHE ESTA TELA ATE PAGAR! Deixe aberta para liberar automatico!</div>
<div class="plano ativo" onclick="sel(this,'2.00',60)"><span>1 HORA</span><b>R$ 2,00</b></div>
<div class="plano" onclick="sel(this,'5.00',120)"><span>2 HORAS</span><b>R$ 5,00</b></div>
<div class="plano" onclick="sel(this,'12.00',1440)"><span>EVENTO TODO</span><b>R$ 12,00</b></div>
...
<div class="footer">SLS WIFI v6.0 - Liberacao Automatica</div>
</div>
<script>
let plano={valor:'2.00',tempo:60};
...
async function gerar(){
  ...
  area.innerHTML='<h3>Gerando PIX Real...</h3>';
  try{
    ...
    console.log(j);
    if(j.imagem){
      area.innerHTML='<h3 style=color:green>PIX REAL GERADO</h3><img src='+j.imagem+'><textarea style=width:100%;height:70px;font-size:10px;margin-top:10px>'+j.copia_e_cola+'</textarea><p id=status style=background:#ffeb3b;padding:10px;border-radius:8px;margin-top:10px> Aguardando pagamento...</p>';
    } else {
      area.innerHTML='<p style=color:red>ERRO:<br>'+JSON.stringify(j)+'</p>';
    }
  }catch(e){area.innerHTML='ERRO JS: '+e.message}
}
</script>
</body>
</html>
...
app.post('/criar-pix', async (req, res) => {
  try {
    ...
    console.log('Cert existe?', fs.existsSync(CERT_PATH));
    const efipay = new EfiPay(efiOptions);
    const body = {
      calendario: { expiracao: 3600 },
      devedor: { nome: 'Cliente SLS' },
      valor: { original: req.body.valor.toString() },
      chave: process.env.EFI_CHAVE_PIX,
      infoAdicionais: [{ nome: 'Plano', valor: req.body.tempo + ' min' }]
    };
    console.log('Body Efí:', body);
