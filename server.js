...
app.post('/criar-pix', async (req, res) => {
  try {
    ...
    console.log('QR OK - TXID:', cob.txid);
    fila.push({ txid: cob.txid, tempo: req.body.tempo, status: 'AGUARDANDO', valor: req.body.valor });
    res.json({ txid: cob.txid, imagem: qrcode.imagemQrcode, copia_e_cola: qrcode.qrcode });
  } catch (err) {
    console.log('===== ERRO CRIAR PIX =====');
...
app.get('/status/:txid', async (req,res)=>{
  try{
    ...
    console.log('STATUS CHECK:', req.params.txid, '->', consulta.status);
    if(consulta.status === 'CONCLUIDA'){
      console.log('✅✅✅ PAGAMENTO CONFIRMADO! TXID:', req.params.txid, 'VALOR:', consulta.valor.original);
      let item = fila.find(f=>f.txid === req.params.txid);
      if(item){ 
        item.status = 'PAGO_LIBERAR';
        console.log('FILA ATUALIZADA PARA PAGO_LIBERAR');
      }
    }
    res.json(consulta);
  }catch(e){
...
app.get('/fila',(req,res)=>{
  console.log('Mikrotik buscou fila - tamanho:', fila.length);
  res.json(fila);
});
