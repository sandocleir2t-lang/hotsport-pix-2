...
return res.send('<html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{background:#0a0a12;color:#fff;text-align:center;padding:15px;font-family:Arial}.qr{background:#fff;padding:15px;border-radius:15px;width:280px;margin:15px auto}.input{width:280px;background:#0f0f1a;border:1px solid #2a2a3a;color:#fff;padding:12px;border-radius:10px;margin:6px 0;box-sizing:border-box}.btn2{width:280px;background:#1e1e2d;border:1px solid #3a3a4a;color:#fff;padding:12px;border-radius:10px;margin-top:8px}.card2{background:#15151f;border-radius:15px;padding:12px;width:280px;margin:15px auto;border:1px solid #222}</style></head><body><h2>SLS WIFI - '+p.nome+' R$ '+p.valor+'</h2><div class="qr"><img src="'+qr.imagemQrcode+'" style="width:100%"><p style="color:#000;font-size:9px;word-break:break-all">'+qr.qrcode+'</p></div><p id="s">Aguardando PIX...</p><div class="card2"><p style="font-size:12px;color:#8b8fa3">JA PAGOU OU TEM VOUCHER?</p><form action="http://10.5.50.1/login" method="post"><input type="hidden" name="dst" value="https://www.google.com"><input class="input" name="username" placeholder="Codigo / Voucher"><input class="input" type="password" name="password" placeholder="Senha"><button class="btn2" type="submit">ENTRAR COM VOUCHER</button></form><button class="btn2" onclick="window.location.href=\'/?mac='+(mac||'')+'&ip='+(ip||'')+'\'">JA PAGUEI - VOLTAR</button></div><script>let t="'+cob.txid+'";setInterval(async()=>{let r=await fetch("/status/"+t);let d=await r.json();if(d.status=="CONCLUIDA"){document.getElementById("s").innerHTML="PAGO! LIBERADO!";setTimeout(()=>{window.location.href="http://10.5.50.1";},1500);}},3000);</script></body></html>');    }catch(e){ return res.send('Erro PIX: '+e.message); }
  }
